...
def pca(features):
	"""
	Adds the two PCA components to the features table.
	"""
...
def plot_killed_wounded_pca(features):
	"""
	Plots the PCA components for the 'Killed' and 'Wounded' fields of the features table.
	"""

	plt.figure(figsize=(20, 5))
	ax = plt.subplot(1, 2, 1)
	features.plot('pca1', 'pca2', kind='scatter', c='Killed', cmap='Reds', ax=ax)
	plt.ylabel('\n Second component')
	plt.xlabel('\n First component')
	plt.title('PCA for number of killed people')
	ax = plt.subplot(1, 2, 2)
	features.plot('pca1', 'pca2', kind='scatter', c='Wounded', cmap='Blues', ax=ax)
	plt.ylabel('\n Second component')
	plt.xlabel('\n First component')
	plt.title('PCA for number of wounded people')
	plt.tight_layout()
	plt.savefig('killed_wounded_pca.pdf')
	plt.show()
...
def plot_attack_weapon_target_type(features, list_unique, list_):
	"""
	Plots the PCA components for the 'Attack_type', 'Target_type' and 'Weapon_type' fields of the features table.
	"""
	...
	plt.scatter(x=features['pca1'], y=features['pca2'], c=np.array(cvec_attack_type), edgecolor='')
	handles_1 = create_handles(label_color_dict_attack_type, ax_1)
	labels_1 = [h.get_label() for h in handles_1]
	plt.ylabel('\n Second component')
	plt.xlabel('\n First component')
	plt.title('PCA for attack type')
	...
	plt.scatter(x=features['pca1'], y=features['pca2'], c=np.array(cvec_weapon_type), edgecolor='')
	handles_2 = create_handles(label_color_dict_weapon_type, ax_2)
	labels_2 = [h.get_label() for h in handles_2] 
	plt.ylabel('\n Second component')
	plt.xlabel('\n First component')
	plt.title('PCA for weapon type')
	...
	plt.scatter(x=features['pca1'], y=features['pca2'], c=np.array(cvec_target_type), edgecolor='')
	handles_3 = create_handles(label_color_dict_target_type, ax_3)
	labels_3 = [h.get_label() for h in handles_3]
	plt.ylabel('\n Second component')
	plt.xlabel('\n First component')
	plt.title('PCA for target type')
	lgd3=ax_3.legend(loc='upper left', bbox_to_anchor=(1, 1), handles=handles_3, labels=labels_3)  

	plt.tight_layout()
	plt.savefig('attack_weapon_target_pca.pdf', bbox_inches='tight')
	plt.show()